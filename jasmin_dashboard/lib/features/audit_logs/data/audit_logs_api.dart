...
final auditLogsApiProvider =
    Provider<AuditLogsApi>((ref) => AuditLogsApi(ref.watch(dioProvider)));

class AuditLogsApi {
  const AuditLogsApi(this._dio);
  final Dio _dio;

  Future<AuditLogsResponse> fetchLogs(
      {required int page,
      int perPage = 50,
      String? action,
      String? targetType,
      String? adminEmail}) async {
    final response = await _dio.get<Map<String, dynamic>>(ApiPaths.auditLogs,
