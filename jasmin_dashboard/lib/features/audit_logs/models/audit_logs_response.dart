...
class AuditLogsResponse {
  const AuditLogsResponse({required this.logs, required this.total, required this.page, required this.perPage, required this.totalPages});
  ...
  factory AuditLogsResponse.fromJson(Map<String, dynamic> json) => AuditLogsResponse(
        logs: (json['logs'] as List? ?? const <dynamic>[]).whereType<Map>().map((item) => AuditLogModel.fromJson(Map<String, dynamic>.from(item))).toList(),
