...
final auditLogsProvider =
    StateNotifierProvider.autoDispose<AuditLogsController, AuditLogsState>(
        (ref) {
  final controller =
      AuditLogsController(ref.watch(auditLogsRepositoryProvider));
...
class AuditLogsController extends StateNotifier<AuditLogsState> {
  ...
  Future<void> load({bool reset = false}) async {
    ...
    state = state.copyWith(
        isLoading: reset && state.logs.isEmpty,
        isLoadingMore: !reset,
        clearError: true);
    try {
      ...
      state = state.copyWith(
          logs: reset ? response.logs : [...state.logs, ...response.logs],
  ...
  Future<void> applyFilters(
      {String? action, String? targetType, String? adminEmail}) async {
    state = state.copyWith(
        action: action ?? state.action,
        targetType: targetType ?? state.targetType,
        adminEmail: adminEmail ?? state.adminEmail,
        clearError: true);
