...
class CustomersApi {
  ...
  Future<CustomersResponse> fetchCustomers({String? query}) async {
    final response = await _dio.get<Map<String, dynamic>>(ApiPaths.customers,
        queryParameters: <String, dynamic>{
          if (query != null && query.trim().isNotEmpty) 'q': query.trim()
        });
    return CustomersResponse.fromJson(
        response.data ?? const <String, dynamic>{});
  }

  Future<CustomerDetailModel> fetchCustomer(String key) async {
    final response =
        await _dio.get<Map<String, dynamic>>(ApiPaths.customerDetail(key));
  ...
  Future<void> setBan(String key,
    ...
    await _dio.patch<Map<String, dynamic>>(ApiPaths.customerDetail(key),
        data: <String, dynamic>{
          'ban': banned,
          if (banned) 'reason': reason ?? ''
        });
  }
}
