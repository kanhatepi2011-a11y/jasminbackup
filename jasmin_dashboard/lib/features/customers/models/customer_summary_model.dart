class CustomerSummaryModel {
  ...
  String get displayName => nickname?.trim().isNotEmpty == true ? nickname!.trim() : email ?? phone ?? key;
  String get subtitle => [email, phone].where((v) => v != null && v.trim().isNotEmpty).join(' • ');

  factory CustomerSummaryModel.fromJson(Map<String, dynamic> json) => CustomerSummaryModel(
  ...
  static double _doubleFrom(dynamic value) => value is double ? value : value is num ? value.toDouble() : double.tryParse(value?.toString() ?? '') ?? 0;
}
