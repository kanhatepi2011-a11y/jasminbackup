...
class _FaqEditorScreenState extends ConsumerState<FaqEditorScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return AdminScaffold(
      ...
      currentRoute: '/faqs',
      actions: [IconButton(tooltip: 'Back to FAQ', onPressed: () => context.go('/faqs'), icon: const Icon(Icons.close_rounded))],
      child: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            Row(children: [
              IconButton.filledTonal(tooltip: 'Back', onPressed: () => context.go('/faqs'), icon: const Icon(Icons.arrow_back_rounded)),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(widget.isEditing ? 'Edit FAQ' : 'Create FAQ', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900)), const SizedBox(height: 4), Text('FAQ changes are saved through secure admin API and reflected on the public FAQ page automatically.', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Colors.black54))])),
            ]),
            const SizedBox(height: 16),
            if (state.errorMessage != null) _NoticeCard(icon: Icons.error_outline_rounded, title: 'FAQ action failed', message: state.errorMessage!, color: Theme.of(context).colorScheme.error),
            ...
            if (state.isLoading)
              ...
              Form(
                ...
                child: _FormCard(title: 'FAQ item', children: [
                  TextFormField(controller: _questionController, minLines: 2, maxLines: 3, decoration: const InputDecoration(labelText: 'Question', prefixIcon: Icon(Icons.question_answer_rounded)), validator: _required('Question')),
                  const SizedBox(height: 14),
                  TextFormField(controller: _answerController, minLines: 5, maxLines: 10, decoration: const InputDecoration(labelText: 'Answer', alignLabelWithHint: true, prefixIcon: Icon(Icons.article_rounded)), validator: _required('Answer')),
                  const SizedBox(height: 14),
                  TextFormField(controller: _categoryController, textInputAction: TextInputAction.next, decoration: const InputDecoration(labelText: 'Category', hintText: 'general, payment, order...', prefixIcon: Icon(Icons.category_rounded)), validator: _required('Category')),
                  const SizedBox(height: 14),
                  ...
                  const SizedBox(height: 14),
                  SwitchListTile.adaptive(contentPadding: EdgeInsets.zero, title: const Text('Visible on FAQ page'), subtitle: const Text('Hidden FAQ items are not shown publicly.'), value: _active, onChanged: state.isSaving ? null : (value) => setState(() => _active = value)),
                  const SizedBox(height: 18),
                  FilledButton.icon(onPressed: state.isSaving ? null : () => _save(controller), icon: state.isSaving ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.save_rounded), label: Text(state.isSaving ? 'Saving...' : 'Save FAQ')),
                ]),
              ),
          ],
        ),
  ...
  Future<void> _save(FaqEditorController controller) async {
    ...
    final result = await controller.save(FaqPayload(question: _questionController.text, answer: _answerController.text, category: _categoryController.text, active: _active, sortOrder: int.tryParse(_sortOrderController.text.trim()) ?? 0));
    if (result != null && mounted) context.go('/faqs');
  }
