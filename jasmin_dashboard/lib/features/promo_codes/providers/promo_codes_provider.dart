...
final promoCodesProvider =
    StateNotifierProvider.autoDispose<PromoCodesController, PromoCodesState>(
        (ref) {
  final controller =
      PromoCodesController(ref.watch(promoCodesRepositoryProvider));
  controller.load();
...
class PromoCodesController extends StateNotifier<PromoCodesState> {
  ...
  Future<void> load({bool silent = false}) async {
    state = state.copyWith(
        isLoading: !silent && state.codes.isEmpty,
        isRefreshing: silent || state.codes.isNotEmpty,
        clearError: true,
        clearSuccess: true);
    try {
      ...
      state = state.copyWith(
          isLoading: false, isRefreshing: false, errorMessage: _message(error));
    }
  }

  Future<void> refresh() => load(silent: true);
  void setQuery(String value) => state =
      state.copyWith(query: value.trim(), clearError: true, clearSuccess: true);
  void setActiveFilter(String value) => state =
      state.copyWith(activeFilter: value, clearError: true, clearSuccess: true);
  void setTypeFilter(String value) => state =
      state.copyWith(typeFilter: value, clearError: true, clearSuccess: true);

  Future<void> toggleActive(PromoCodeModel code) async {
    state = state.copyWith(
        actionCodeId: code.id, clearError: true, clearSuccess: true);
    try {
    ...
    try {
      ...
      state = state.copyWith(
          ...
          successMessage: deleted
              ? '${code.code} deleted.'
              : '${code.code} has orders, so it was disabled instead of deleted.');
      await load(silent: true);
    } catch (error) {
      if (!mounted) {
        return;
  ...
  void startAutoRefresh() {
    _timer?.cancel();
    _timer = Timer.periodic(RefreshIntervals.contentManagement, (_) {
      if (mounted) {
        load(silent: true);
      }
    });
  ...
  String _message(Object error) => error is AppException
      ? error.message
      : 'Promo code action failed. Please try again.';
...
class PromoCodesState {
  ...
  List<PromoCodeModel> get filteredCodes {
    ...
    return codes.where((code) {
      final activeOk = activeFilter == 'ALL' ||
          (activeFilter == 'ACTIVE' && code.active) ||
          (activeFilter == 'INACTIVE' && !code.active) ||
          (activeFilter == 'USABLE' && code.canBeUsed) ||
          (activeFilter == 'EXPIRED' && code.isExpired);
      final typeOk =
          typeFilter == 'ALL' || code.discountType.toUpperCase() == typeFilter;
      final textOk = text.isEmpty ||
          [
            code.code,
            code.discountType,
            code.discountValue.toString(),
            code.minOrderUsd.toString()
...
final promoCodeEditorProvider = StateNotifierProvider.autoDispose
    .family<PromoCodeEditorController, PromoCodeEditorState, String?>(
        (ref, promoId) {
  final controller = PromoCodeEditorController(
      ref.watch(promoCodesRepositoryProvider), promoId);
  controller.load();
  return controller;
});

class PromoCodeEditorController extends StateNotifier<PromoCodeEditorState> {
  PromoCodeEditorController(this._repository, this.promoId)
      : super(const PromoCodeEditorState());
  final PromoCodesRepository _repository;
  ...
  Future<PromoCodeModel?> save(PromoCodePayload payload) async {
    ...
    try {
      final result = isEditing
          ? await _repository.updatePromoCode(promoId!, payload)
          : await _repository.createPromoCode(payload);
      if (!mounted) {
        return result;
      }
      state = state.copyWith(
  ...
  String _message(Object error) =>
      error is AppException ? error.message : 'Promo code save failed.';
}

class PromoCodeEditorState {
  const PromoCodeEditorState(
      {this.code,
      this.isLoading = false,
      this.isSaving = false,
      this.errorMessage,
