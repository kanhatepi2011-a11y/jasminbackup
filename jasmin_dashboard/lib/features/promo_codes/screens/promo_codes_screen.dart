...
class _PromoCodesScreenState extends ConsumerState<PromoCodesScreen> {
    ...
    return AdminScaffold(
      ...
      actions: [
        ...
        IconButton.filledTonal(
            ...
            onPressed: () => context.go('/promo-codes/new'),
        ...
        child: ListView(
          ...
          children: [
            _Header(
                ...
                usable: state.codes.where((c) => c.canBeUsed).length,
                lastUpdatedAt: state.lastUpdatedAt),
            const SizedBox(height: 16),
            TextField(
              controller: _searchController,
              decoration: InputDecoration(
                  hintText: 'Search code, type, discount, minimum order...',
            ...
            const SizedBox(height: 16),
            if (state.errorMessage != null && state.codes.isEmpty)
              _ErrorCard(
            ...
            if (state.isLoading)
              ...
              const EmptyState(
                  ...
                  message: 'Create a promo code or adjust your filters.')
            else
              for (final code in codes)
                PromoCodeListTile(
                    code: code,
                    isBusy: state.actionCodeId == code.id,
                    onToggleActive: () => controller.toggleActive(code),
                    onDelete: () => _confirmDelete(context, controller, code)),
          ],
        ),
      ),
    );
  ...
  Future<void> _confirmDelete(BuildContext context,
      PromoCodesController controller, PromoCodeModel code) async {
    final ok = await showDialog<bool>(
            context: context,
            builder: (context) => AlertDialog(
                    title: const Text('Delete promo code?'),
                    content: Text(
                        'If “${code.code}” has existing orders, backend will disable it instead of deleting it.'),
                    actions: [
                      TextButton(
                      ...
                      FilledButton(
                          ...
                          child: const Text('Delete / disable'))
                    ])) ??
        false;
    if (ok) {
      await controller.deletePromoCode(code);
    }
  }
}
...
class _Header extends StatelessWidget {
  const _Header(
      {required this.total, required this.usable, required this.lastUpdatedAt});
  final int total;
  final int usable;
          ...
          child: Row(children: [
            ...
            Expanded(
                child: Column(
                  ...
                  Text(
                      '$usable usable • $total total • Updated ${Formatters.dateTimeOrDash(lastUpdatedAt)}',
                      style: Theme.of(context)
                          .textTheme
                          .bodySmall
  ...
  Widget build(BuildContext context) => Card(
      child: Padding(
          ...
          child:
            ...
            Text('Could not load promo codes',
                style: Theme.of(context)
                    .textTheme
                    .titleMedium
                    ?.copyWith(fontWeight: FontWeight.w900)),
